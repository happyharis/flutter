  ...
  testUsingContext('determine does not call fetch --tags', () {
    ...
    when(processUtils.runSync(
      ...
      environment: anyNamed('environment'),
        ...
        RunResult(ProcessResult(105, 0, '', ''), <String>['git', 'fetch']));
    when(processUtils.runSync(
      <String>[
        'git',
        'describe',
      ...
      workingDirectory: anyNamed('workingDirectory'),
      environment: anyNamed('environment'),
    )).thenReturn(RunResult(ProcessResult(106, 0, 'v0.1.2-3-1234abcd', ''),
        <String>['git', 'describe']));
    when(processUtils.runSync(
  ...
  testUsingContext('determine does not fetch tags on dev/stable/beta', () {
    ...
    when(processUtils.runSync(
      ...
      environment: anyNamed('environment'),
        ...
        RunResult(ProcessResult(105, 0, 'dev', ''), <String>['git', 'fetch']));
    when(processUtils.runSync(
      <String>[
        'git',
        'fetch',
      ...
      environment: anyNamed('environment'),
        ...
        RunResult(ProcessResult(106, 0, '', ''), <String>['git', 'fetch']));
    when(processUtils.runSync(
      <String>[
        'git',
        'describe',
      ...
      workingDirectory: anyNamed('workingDirectory'),
      environment: anyNamed('environment'),
    )).thenReturn(RunResult(ProcessResult(107, 0, 'v0.1.2-3-1234abcd', ''),
        <String>['git', 'describe']));
    when(processUtils.runSync(
      <String>['git', 'tag', '--points-at', 'HEAD'],
      workingDirectory: anyNamed('workingDirectory'),
  ...
  testUsingContext('determine calls fetch --tags on master', () {
    ...
    when(processUtils.runSync(
      ...
      environment: anyNamed('environment'),
        ...
        ProcessResult(108, 0, 'master', ''), <String>['git', 'fetch']));
    when(processUtils.runSync(
      <String>[
        'git',
        'fetch',
      ...
      environment: anyNamed('environment'),
        ...
        RunResult(ProcessResult(109, 0, '', ''), <String>['git', 'fetch']));
    when(processUtils.runSync(
      <String>['git', 'tag', '--points-at', 'HEAD'],
      workingDirectory: anyNamed('workingDirectory'),
      environment: anyNamed('environment'),
    ...
    when(processUtils.runSync(
      ...
      workingDirectory: anyNamed('workingDirectory'),
      environment: anyNamed('environment'),
    )).thenReturn(RunResult(ProcessResult(111, 0, 'v0.1.2-3-1234abcd', ''),
        <String>['git', 'describe']));
...
void fakeData(
  ...
  when(cache.setStampFor(any, any)).thenAnswer((Invocation invocation) {
    ...
    if (expectSetStamp) {
      stamp = VersionCheckStamp.fromJson(castStringKeyedMap(
          json.decode(invocation.positionalArguments[1] as String)));
      return;
    }

    throw StateError(
        'Unexpected call to Cache.setStampFor(${invocation.positionalArguments}, ${invocation.namedArguments})');
  });
  ...
  final Answering<ProcessResult> syncAnswer = (Invocation invocation) {
    bool argsAre(String a1,
      ...
      final List<String> expectedArgs = <String>[
        a1,
        a2,
        a3,
        a4,
        ...
        a8,
        a9
      ].where((String arg) => arg != null).toList();
    ...
    if (listArgsAre(FlutterVersion.gitLog(
        <String>['-n', '1', '--pretty=format:%ad', '--date=iso']))) {
      ...
      // Careful here!  argsAre accepts 9 arguments and FlutterVersion.gitLog adds 4.
        ...
        listArgsAre(FlutterVersion.gitLog(<String>[
